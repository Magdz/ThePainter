    ...
    private void Draw(){
        switch (ShapeType) {
            case "Triangle":
                ...
                break;
            case "Rectangle":
                Shape Rectangle = new Rectangle(drawStart.x,drawStart.y,drawEnd.x,drawEnd.y,Color);
                this.shape = Rectangle;
                canvas.addShape(Rectangle);
                break;
            case "Square":
                Shape Square = new Square(drawStart.x,drawStart.y,drawEnd.x,drawEnd.y, Color);
                this.shape = Square;
                canvas.addShape(Square);
                break;
            case "Ellipse":
                Shape Ellipse = new Elipse(Color,Color,5,Math.min(drawStart.x, drawEnd.x),Math.min(drawStart.y, drawEnd.y), Math.abs(drawStart.y-drawEnd.y), Math.abs((drawStart.x-drawEnd.x)));
                this.shape = Ellipse;
                canvas.addShape(Ellipse);
                break;
            case "Circle":
                Shape Circle = new Circle(Color,Color,1,Math.min(drawStart.x, drawEnd.x),Math.min(drawStart.y, drawEnd.y), Math.abs((drawStart.x-drawEnd.x)/2));
                this.shape = Circle;
                canvas.addShape(Circle);
                break;
            case "Line":
                Shape Line = new Line(Color,Color,5,drawStart.x,drawEnd.x,drawStart.y,drawEnd.y);
                this.shape = Line;
